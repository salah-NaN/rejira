            ...
            xl:gap-10">
                {
                    ...
                    projects && projects.map(project => {
                        return (
                            <div className="select-none relative h-56 px-7 py-4 shadow-lg
                              bg-[#3b82f6]/80 "
                                key={project.id}
                                onClick={() => handleProject(project.id)}>
                                <div className="flex justify-between items-center mb-4" >
                                    <h3 className="text-2xl font-semibold truncate text-[#f9f9f9] capitalize" >{project.title}</h3>
                                    <div className={`size-5 ml-3 bg-${project.active ? 'green' : 'red'}-500/80 rounded-full shrink-0 text-pretty`} ></div>
                                </div>
                                <p className="text-[13px] text-pretty text-[#dadada] truncate line-clamp-4 cursor-default" >{project.description}</p>
